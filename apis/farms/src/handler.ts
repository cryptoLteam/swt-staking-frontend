...
import { DUSD } from '@pancakeswap/tokens'
...
const cakeBusdPairMap = {
  [ChainId.DYNO]: {
    address: Pair.getAddress(CAKE[ChainId.DYNO], BUSD[ChainId.DYNO]),
    tokenA: CAKE[ChainId.DYNO],
    tokenB: BUSD[ChainId.DYNO],
  },
  [ChainId.DYNO_TESTNET]: {
    address: Pair.getAddress(CAKE[ChainId.DYNO_TESTNET], BUSD[ChainId.DYNO_TESTNET]),
    tokenA: CAKE[ChainId.DYNO_TESTNET],
    tokenB: BUSD[ChainId.DYNO_TESTNET],
  },
}

const getCakePrice = async (isTestnet: boolean) => {
  const pairConfig = cakeBusdPairMap[ChainId.DYNO]
  ...
  try {
    ...
    const finalFarm = farmsWithPrice.map((f) => {
      return {
        ...
        cakeApr: getFarmCakeRewardApr(f, FixedNumber.from(cakeBusdPrice.toSignificant(3)), rewardPerBlock),
      }
    }) as FarmResult
